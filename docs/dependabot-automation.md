...
- Required check: `test` (job name from [CI](../.github/workflows/ci.yml)). Do **not** require `Build & push image`; that job from [Build & Push Container Image](../.github/workflows/build.yml) runs only on `master` pushes and `workflow_dispatch`.
